@include('Templates.Frontend.head')
@include('Templates.Frontend.header')

<div class="container">
    <div class="row">
        <div class="box">

            <div class="col-lg-12">
                <div class="visitNav">
                    <div>
                        <h2 class="text-center">Agencias de Turismo</h2>
                    </div>
                </div>
                <hr class="tagline-divider-cont">
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">Rainbow Travel Buga</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>CALLE 6 No. 16 – 21 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>316 618 7865 - 316 5329690</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">

                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">On Vacation</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia1.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>Calle 7 10-51 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>318 2585951</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">Colombia Tours</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia2.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>Carrera. 13 No. 10-67 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>2391947</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">American WORK CO.</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia3.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>Calle 6 No. 11-48 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>315 2403102</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">Explora Tu País</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia4.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>Calle 7 # 9-52 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>315 5157226</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">Viajes San Francisco</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia5.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>Calle 5 No 13-67 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>(2) 2360023</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">Nuestra Tierra</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia6.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>carrera 16 # 5-33 piso 1 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>5722394185 - 316 6187865</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>
            <div>
                <div class="row">
                    <div class="col-md-8">
                        <div class="page-header">
                            <div class="blog">
                                <h5 class="exp">TurisMundo Buga</h5>
                                <div class="">
                                    <img class="thumbnail img-responsive img-agen" src="{{asset('img/agencia7.jpg')}}" alt="" />			
                                </div>
                <!--						<p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit,
                    sed diam nonummy nibh euismod tincidunt ut laoreet dolore magna aliquam erat volutpat.
                    Ut wisi enim ad minim veniam,quis nostrud exerci tation ullamcorper
                    suscipit lobortis nisl ut aliquip ex ea commodo consequat.</p>-->
                            </div>
                        </div>				
                    </div>


                    <div class="col-md-4">
                        <div class="panel panel-default panel-edit">
                            <div class="panel-heading">
                                <strong>Datos</strong>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">DIRECCION:</h4>
                                        <p>Carrera 9 Nº 9 - 46 Buga</p>
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">TELEFONO:</h4>
                                        <p>315 746 6420</p>							
                                    </div>
                                </div>
                            </div>
                            <div class="panel-body">
                                <div class="media">
                                    <div class="media-body">
                                        <h4 class="media-heading">CORREO:</h4>
                                        <p></p>
                                    </div>
                                </div>
                            </div>
                        </div>			
                    </div>				
                </div>
            </div>

            @include('Templates.Frontend.footer')
            @include('Templates.Frontend.foot')


